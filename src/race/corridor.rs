...
impl CorridorScroller {
    ...
    pub fn update(&mut self) {
        ...
        self.x += match self.dir {
            ScrollDir::Left => -self.speed,
            ScrollDir::Right => self.speed,
        } * dt;
        if self.x <= -self.w {
            self.x += self.w;
            self.loops_done += 1;
    ...
    pub fn finished(&self) -> bool {
        self.loops_done >= self.loops_goal
    }
    ...
    pub fn draw(&self) {
        ...
        let params = DrawTextureParams {
            dest_size: Some(vec2(self.w, full_h)),
            .. Default::default()
        };

        // draw the two tiles, each stretched to full_h
        draw_texture_ex(&self.tex, self.x,       0.0, WHITE, params.clone());
        let other_x = if self.dir == ScrollDir::Left {
            self.x + self.w
        } else {
            self.x - self.w
        };
        draw_texture_ex(&self.tex, other_x, 0.0, WHITE, params);
    }
}
